...
namespace FairyNails
{
    public partial class ServicesControl : UserControl
    {
        ...
        {
            if (MessageBox.Show("Xác nhận xóa?", "Delete Type of Service", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
                ServiceType type = (from s in db.ServiceTypes
                                    where s.id == typeofserviceId
                                    select s).Single<ServiceType>();
                // delete all works
                var works = from w in db.Works
                            where w.service_id == serviceId
                            select w;
                db.Works.DeleteAllOnSubmit(works);
